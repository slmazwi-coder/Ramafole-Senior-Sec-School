...
export const Navbar = () => {
  ...
  return (
    <nav className="w-full sticky top-0 z-50" style={ { background: '#1B2A4A', borderBottom: '3px solid #C4A862' } }>

      {/* ── Top bar: Logo + School name + Student Portal ── */}
      <div className="w-full" style={ { borderBottom: '1px solid rgba(196,168,98,0.3)' } }>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">

            {/* Logo + Name */}
            <Link to="/" className="flex items-center gap-3 min-w-0 flex-1">
              <div className="h-11 w-11 shrink-0 rounded-xl bg-white flex items-center justify-center overflow-hidden shadow-md" style={ { border: '2px solid #C4A862' } }>
                <img
                  src="/rslogo.png"
                  alt="Ramafole SSS logo"
              ...
              <Link
                ...
                className={cn(
                  ...
                  location.pathname.startsWith('/student')
                    ? 'text-[#1B2A4A] bg-[#C4A862]'
                    : 'text-[#C4A862] border-2 border-[#C4A862] hover:bg-[#C4A862] hover:text-[#1B2A4A]'
                )}
              >
              ...
              <Link
                ...
                style={
                  location.pathname === link.path
                    ? { color: '#1B2A4A', background: '#C4A862', fontWeight: 700 }
                    : { color: '#F5F0DC' }
                }
                onMouseEnter={e => {
                  if (location.pathname !== link.path) {
                    (e.target as HTMLElement).style.background = 'rgba(196,168,98,0.15)';
                  }
                }}
                onMouseLeave={e => {
                  if (location.pathname !== link.path) {
      ...
      {/* ── Mobile dropdown ── */}
      {isOpen && (
        <div className="md:hidden shadow-lg" style={ { background: '#1B2A4A', borderTop: '1px solid rgba(196,168,98,0.3)' } }>
          <div className="px-3 pt-2 pb-4 space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                style={
                  location.pathname === link.path
                    ? { background: '#C4A862', color: '#1B2A4A', fontWeight: 700 }
                    : { color: '#F5F0DC' }
                }
              >
                {link.name}
            ...
            <Link
              to="/student/login"
              onClick={() => setIsOpen(false)}
              className="block px-4 py-3 mt-2 rounded-lg text-sm font-bold text-center transition-colors"
              style={ { border: '2px solid #C4A862', color: '#C4A862' } }
            >
              Student Portal
            </Link>
          </div>
